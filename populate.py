...
def main():
    ...
    try:
        ...
        for root, dirs, files in os.walk(csvDir):
            ...
            for file_name in files:
                ...
                if file_name.endswith(".csv"):
                    ...
                    sql = generate_sql(file_path, db_name, table_name[2:],
                                       fields)
                    print sql
                    c.execute(sql)
        ...
        # create publication table and bring in other tables
        populate_pub_table(db.cursor())
        db.commit()
...
def create_tables(conn):
    tables = [
        ...
        " FOREIGN KEY (entity_id) REFERENCES entity (eid))," \
        "CREATE TABLE publications (eid INTEGER, title VARCHAR(1000), month VARCHAR(50), year INTEGER," \
        "   PRIMARY KEY (eid));"
    ]
...
# create the publication table and populate it
def populate_pub_table(c):
    c.execute('''
-- bring in all the individual publication tables with relevent data
